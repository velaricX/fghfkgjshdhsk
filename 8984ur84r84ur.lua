--[[
	...
	Lumu Hub Dex is a revival of Moon's Dex, made to fulfill Moon's Dex prophecy.
]]
...
-- Common Locals
local Main,Lib,Apps,Settings -- Main Containers
local Explorer, Properties, ScriptViewer, ModelViewer, DecalPreview, AnimationViewer, MeshViewer, Favorites, Notebook -- Major Apps
local API,RMD,env,service,plr,create,createSimple -- Main Locals
...
local function main()
	...
	Explorer.ShowRightClick = function(MousePos)
		...
		if #sList == 1 then
			if Favorites.IsFavorited(sList[1].Obj) then
				context:AddRegistered("REMOVE_FAVORITE")
			else
				context:AddRegistered("ADD_FAVORITE")
			end
		end
		...
		if presentClasses["MeshPart"] or presentClasses["SpecialMesh"] then
			context:AddRegistered("MESH_PREVIEW")
		end
	...
	Explorer.InitRightClick = function()
		...
		context:Register("ADD_FAVORITE",{Name = "Add to Favorites", IconMap = Explorer.MiscIcons, Icon = "AddStar", DisabledIcon = "Empty", OnClick = function()
			local sList = selection.List
			if #sList == 1 then
				Favorites.Add(sList[1].Obj)
			end
		end})

		context:Register("REMOVE_FAVORITE",{Name = "Remove from Favorites", IconMap = Explorer.MiscIcons, Icon = "RemoveStar", DisabledIcon = "Empty", OnClick = function()
			local sList = selection.List
			if #sList == 1 then
				Favorites.Remove(sList[1].Obj)
			end
		end})
		...
		local meshIconInd = (RMD.Classes["MeshPart"] and tonumber(RMD.Classes["MeshPart"].ExplorerImageIndex)) or 54

		context:Register("MESH_PREVIEW",{Name = "Mesh Preview", IconMap = Explorer.ClassIcons, Icon = meshIconInd, OnClick = function()
			local sList = selection.List
			local isa = game.IsA

			if #sList == 1 then
				if isa(sList[1].Obj,"MeshPart") or isa(sList[1].Obj,"SpecialMesh") then
					MeshViewer.ViewMesh(sList[1].Obj)
					return
				end
			end
		end})
...
local function main()
	...
	MeshViewer.ViewMesh = function(inst)
		if not (inst and (inst:IsA("MeshPart") or inst:IsA("SpecialMesh"))) then return end

		local meshId = inst.MeshId
		local sourceObj = inst:IsA("SpecialMesh") and inst.Parent or inst
		if not sourceObj or not sourceObj:IsA("BasePart") then return end

		window:SetTitle(inst.Name.." - Mesh Preview")
		window:Show()
		pathLabel.Gui.Text = "path: " .. getPath(inst)

		cleanup()

		-- some games disable Archivable to block cloning, force it back on
		sourceObj.Archivable = true
		for _,v in ipairs(sourceObj:GetDescendants()) do
			v.Archivable = true
		end

		local ok, clone = pcall(function() return sourceObj:Clone() end)
		if not ok or not clone then
			setInfoText(inst.Name, "Failed to clone mesh", "N/A", "N/A")
			return
		end
		...
		if ok2 and cf then
			...
			camDistance = math.clamp(size.Magnitude, 2, 100)
		else
			camTarget = clone.Position
			camDistance = 10
		end
		orbitYaw = 0
		updateCamera()

		local id = extractAssetId(meshId)
		if not id then
			setInfoText(inst.Name, "Unknown", "N/A", "Unknown")
			return
		end
	...
	MeshViewer.Init = function()
		window = Lib.Window.new()
		window:SetTitle("Mesh Preview")
		window:Resize(300,360)
		MeshViewer.Window = window
		...
		viewport.MouseWheelForward:Connect(function()
			camDistance = math.clamp(camDistance - 1, 2, 100)
			updateCamera()
		end)
		viewport.MouseWheelBackward:Connect(function()
			camDistance = math.clamp(camDistance + 1, 2, 100)
...
["Favorites"] = function()
--[[
	Favorites App Module
]]

-- Common Locals
local Main,Lib,Apps,Settings -- Main Containers
local Explorer, Properties, ScriptViewer, ModelViewer, DecalPreview, AnimationViewer, MeshViewer, Notebook -- Major Apps
local API,RMD,env,service,plr,create,createSimple -- Main Locals
...
local function main()
	...
	local window, listHolder

	-- same trick as Console's Execute
	local function resolvePath(path)
		local ok, fn = pcall(loadstring, "return " .. path)
		if not ok or not fn then return nil end
		local ok2, inst = pcall(fn)
		if not ok2 or typeof(inst) ~= "Instance" then return nil end
		return inst
	end

	Favorites.IsFavorited = function(inst)
		local path = Explorer.GetInstancePath(inst)
		return table.find(Settings.Favorites, path) ~= nil
	end
	...
	Favorites.RefreshList = function()
		...
		for i,path in ipairs(Settings.Favorites) do
			...
			local nameLbl = Lib.Label.new()
			nameLbl.Gui.Parent = row
			nameLbl.Gui.Size = UDim2.new(1,-40,1,0)
			nameLbl.Gui.TextXAlignment = Enum.TextXAlignment.Left
			nameLbl.Gui.TextSize = 13
			nameLbl.Gui.TextTruncate = Enum.TextTruncate.AtEnd
			nameLbl.Gui.Text = inst and inst.Name or "(missing) " .. path
			if not inst then
				nameLbl.Gui.TextTransparency = 0.5
			end

			local goBtn = Instance.new("TextButton")
			goBtn.Parent = row
			goBtn.AnchorPoint = Vector2.new(1,0.5)
			goBtn.Position = UDim2.new(1,-20,0.5,0)
			goBtn.Size = UDim2.new(0,18,0,16)
			goBtn.BackgroundColor3 = Settings.Theme.Button
			goBtn.BorderSizePixel = 0
			goBtn.Text = "→"
			goBtn.TextColor3 = Settings.Theme.Text
			goBtn.TextSize = 13
			...
			removeBtn.Size = UDim2.new(0,18,0,16)
			removeBtn.BackgroundColor3 = Settings.Theme.Button
			removeBtn.BorderSizePixel = 0
			removeBtn.Text = "X"
			removeBtn.TextColor3 = Settings.Theme.Text
	...
	Favorites.Add = function(inst)
		local path = Explorer.GetInstancePath(inst)
		if table.find(Settings.Favorites, path) then return end
		table.insert(Settings.Favorites, path)
		Main.SaveCurrentSettings()
		Favorites.RefreshList()
	end

	Favorites.Remove = function(inst)
		local path = Explorer.GetInstancePath(inst)
		local ind = table.find(Settings.Favorites, path)
		if not ind then return end
	...
	Favorites.Init = function()
		...
		listHolder.Parent = window.GuiElems.Content
		listHolder.BackgroundTransparency = 1
		listHolder.Position = UDim2.new(0,4,0,4)
		listHolder.Size = UDim2.new(1,-8,1,-8)
...
local function main()
	...
	local function AddColorPicker(title, themeKey)
		...
		swatch.BackgroundColor3 = Settings.Theme[themeKey]
		swatch.BorderSizePixel = 1
		swatch.BorderColor3 = Color3.new(0,0,0)
		swatch.Size = UDim2.new(0,30,0,15)
		swatch.Text = ""
		swatch.Parent = frame.Gui
		...
		swatch.MouseButton1Click:Connect(function()
			if not SettingsWindow.ThemeColorEditor then
				local editor = Lib.ColorPicker.new()
				editor.Window:SetTitle("Theme Color Picker")

				editor.OnSelect:Connect(function(col)
					local key = SettingsWindow.ThemeColorEditorKey
					if not key then return end
					Settings.Theme[key] = col
					SettingsWindow.ThemeColorEditorSwatches[key].BackgroundColor3 = col
				end)

				SettingsWindow.ThemeColorEditor = editor
			end

			SettingsWindow.ThemeColorEditorKey = themeKey
			SettingsWindow.ThemeColorEditor:SetColor(Settings.Theme[themeKey])
			SettingsWindow.ThemeColorEditor.Window:ShowAndFocus()
		end)

		SettingsWindow.ThemeColorEditorSwatches = SettingsWindow.ThemeColorEditorSwatches or {}
		SettingsWindow.ThemeColorEditorSwatches[themeKey] = swatch
	...
	SettingsWindow.Init = function()
		...
		AddSeperator("Theme")
		AddText("Requires reload to apply (Save button below).")

		AddColorPicker("Accent / Selection", "ListSelection")
		AddColorPicker("Window Background", "Main1")
		AddColorPicker("Panel Background", "Main2")
		AddColorPicker("Outline", "Outline1")
		AddColorPicker("Button", "Button")
		AddColorPicker("Button Hover", "ButtonHover")
		AddColorPicker("Text", "Text")

		local resetColorsFrame = Lib.Frame.new()
		resetColorsFrame.Gui.Parent = ListFrame
		resetColorsFrame.Gui.Transparency = 1
		resetColorsFrame.Gui.Size = UDim2.new(1,0,0,20)

		local resetColorsBtn = Instance.new("TextButton")
		resetColorsBtn.Parent = resetColorsFrame.Gui
		resetColorsBtn.Size = UDim2.new(1,0,1,0)
		resetColorsBtn.BackgroundColor3 = Settings.Theme.Button
		resetColorsBtn.BorderSizePixel = 0
		resetColorsBtn.Text = "Reset Colors"
		resetColorsBtn.TextColor3 = Settings.Theme.Text
		resetColorsBtn.TextSize = 13

		resetColorsBtn.MouseButton1Click:Connect(function()
			local defaultTheme = {
			...
			for key,col in pairs(defaultTheme) do
				Settings.Theme[key] = col
				local swatch = SettingsWindow.ThemeColorEditorSwatches and SettingsWindow.ThemeColorEditorSwatches[key]
				if swatch then swatch.BackgroundColor3 = col end
			end
		end)
...
-- Main vars
local Main, Explorer, Properties, ScriptViewer, Console, SaveInstance, ModelViewer, DecalPreview, AnimationViewer, MeshViewer, Favorites, SettingsWindow--[[, SecretServicePanel]], DefaultSettings, Notebook, Serializer, Lib local ggv = getgenv or nil
local API, RMD
...
Main = (function()
	...
	Main.ModuleList = {"Explorer","Properties","ScriptViewer","Console","SaveInstance","ModelViewer","DecalPreview","AnimationViewer","MeshViewer","Favorites","SettingsWindow"}
	Main.Elevated = false
	...
	Main.CreateIntro = function(initStatus) -- TODO: Must theme and show errors
		local gui = create({
			...
			{5,"TextLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Font=4,Name="Title",Parent={3},Position=UDim2.new(0,-260,0,20),Size=UDim2.new(0,260,0,40),Text="LumuHubDex",TextColor3=Color3.new(1,1,1),TextSize=32,TextTransparency=1,TextXAlignment=0,}},
			...
			{11,"TextLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Font=3,Name="Creator",Parent={2},Position=UDim2.new(1,-110,1,-20),Size=UDim2.new(0,105,0,20),Text="Made by Velaric",TextColor3=Color3.new(1,1,1),TextSize=14,TextXAlignment=1,}},
	...
	Main.SetMainGuiOpen = function(val)
		...
		Main.MainGui.OpenButton.Text = val and "Close" or "LumuHubDex"
	...
	Main.CreateMainGui = function()
		...
		Main.CreateApp({Name = "Mesh Viewer", IconMap = Main.LargeIcons, Icon = "Watcher", Window = MeshViewer.Window})

		Main.CreateApp({Name = "Favorites", IconMap = Main.LargeIcons, Icon = "Script_Viewer", Window = Favorites.Window})
