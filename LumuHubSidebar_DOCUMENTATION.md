# Lumu UI — Sidebar Design (`LumuHubSidebar.lua`)

**This file is the SIDEBAR design only** (vertical tab list on the left).
For the horizontal top-tab design use `LumuHubTopbar.lua` — same API, see the
other doc. You do **not** pass a `Design` flag; the file itself is the design.

Two files work together:

| File | What |
|---|---|
| `LumuHubSidebar.lua` | UI library only (no demo). Ends with `return Astral`. |
| `icons_sprites.lua` | Icon module: ~1800 game sprites + 92 classic UI icons. Ends with `return SPI`. |

```lua
local Astral = loadstring(game:HttpGet("https://raw.githubusercontent.com/velaricX/fghfkgjshdhsk/main/LumuHubSidebar.lua"))()
...
`LumuUI.lua` is the exact same sidebar build (old loadstring still works).
...
A search box sits at the **right of the window header**, outside the title flow.
Typing filters the current tab's elements by name (matches raw names, displayed
titles and descriptions, in any language). A dropdown lists matches from **all**
tabs as `Name (Tab)` — click one to jump straight to that tab. Clearing the box
restores everything. No code needed, always on.
## 2. Categories & Tabs

```lua
Window:AddCategory("Main Features")          -- gray section header in sidebar
...
local Status = Window:AddGameStatus({
  ...
  Width = 240,                    -- optional panel width in px (default 240 PC / 150 mobile)
...
and selector **option names** also translate for display (values passed to
callbacks stay raw, so nothing breaks). The Discord card (server name,
description, button, Online/Members words), the color picker (`CURRENT` /
`NEW` / `Apply` / `Cancel`), the status panel title + row names and the sidebar
`Minimize` label translate as well — add entries with `AddTranslations()` and
they swap live. Live row values, timer text and option values never translate
(they're data, not labels).
